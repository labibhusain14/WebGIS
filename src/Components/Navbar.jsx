...
const Navbar = () => {
  ...
  const handleNavigation = (name) => {
    ...
    if (
      location.pathname === "/" &&
      !isAuthenticated &&
      restrictedRoutes.includes(targetRoute)
    ) {
      navigate("/login");
    } else {
      setActiveTab(name);
  ...
  return (
    <motion.nav
      className="fixed top-0 left-0 right-0 z-50 bg-white shadow-md font-poppins"
      initial="hidden"
      animate="visible"
    ...
    >
      <div className="max-w-7xl mx-auto px-4 py-3">
        <div className="flex items-center justify-between">
          {/* Logo */}
          <motion.div
            className="flex items-center font-bold text-gray-700 cursor-pointer"
            onClick={() => navigate("/")}
            whileHover={{ scale: 1.03 }}
            whileTap={{ scale: 0.97 }}
          >
            <img src={Logo} alt="Logo" className="w-10 h-10 mr-2" />
            <span className="text-lg bg-blue-600 bg-clip-text text-transparent">
              KOSTHUB
            </span>
          </motion.div>
          ...
          <div className="hidden md:flex gap-8">
            {Object.entries(tabRoutes).map(([name, { icon }]) => (
              <motion.button
                ...
                className={`text-base font-semibold flex items-center gap-2 pb-1 px-2 relative ${
                  activeTab === name
                    ? "text-blue-600"
          ...
          <div className="relative flex items-center" ref={dropdownRef}>
            {isAuthenticated ? (
              <>
                ...
                <AnimatePresence>
                  {showDropdown && (
                    <motion.div
                      className="absolute right-0 mt-2 top-full w-48 bg-white border rounded-lg shadow-lg z-50 overflow-hidden"
                    ...
                    >
                      ...
                      <motion.button
                        ...
                        className="flex items-center gap-2 w-full text-left px-4 py-3 hover:bg-gray-50 "
                        whileHover={{ backgroundColor: "#f3f4f6" }}
                        whileTap={{ scale: 0.98 }}
                      >
                        <FiHeart className="text-lg text-blue-500" />
                        <span>Favorit</span>
                      </motion.button>
            ...
            {/* Hamburger Menu */}
            <motion.button
              className="md:hidden text-2xl text-gray-700 menu-toggle p-2"
        ...
        <AnimatePresence>
          {mobileMenuOpen && (
            <motion.div
              className="md:hidden mt-3 flex flex-col gap-3 overflow-hidden bg-white rounded-lg shadow-lg p-4"
              ref={mobileMenuRef}
            ...
            >
              {Object.entries(tabRoutes).map(([name, { icon }]) => (
                <motion.button
                  key={name}
                  onClick={() => handleNavigation(name)}
                  className={`text-left font-semibold text-base p-3 rounded-lg flex items-center gap-3 ${
                    activeTab === name
              ...
              <div className="flex flex-col gap-2 mt-2">
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  className="flex items-center gap-3 w-full text-left px-4 py-3 rounded-lg text-blue-600 bg-blue-50 hover:bg-blue-100"
                >
