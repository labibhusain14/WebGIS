    ...
    >
      <div className="w-full flex justify-end p-3 md:hidden">
        <motion.button
          ...
          className="text-gray-700 hover:text-red-600 bg-white rounded-full p-1 shadow-md"
          whileTap={{ scale: 0.9 }}
                ...
                {openDropdown === label && (
                  <motion.div
                    className={`absolute left-0 mt-2 ${
                      label === "Price" ? "w-80" : "w-56"
                    } bg-white shadow-lg rounded-lg p-4 text-sm z-50 border border-gray-200`}
                    initial="hidden"
                    animate="visible"
                    exit="hidden"
      ...
      <AnimatePresence>
        {loading && (
          ...
          >
            <motion.div
              className="h-8 w-8 border-t-4 border-blue-600 border-solid rounded-full"
              animate={{ rotate: 360 }}
      ...
      <AnimatePresence>
        {filteredKost.length > 0 && (
          <motion.div
            className="w-[90%] mx-auto flex justify-center items-center py-4 mt-2"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
      ...
      <AnimatePresence>
        {filteredKost.length === 0 && !loading && (
          <motion.div
            className="w-[90%] mx-auto py-8 flex flex-col items-center justify-center text-center"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -20 }}
          >
