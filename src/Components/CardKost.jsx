...
import { FaParking, FaTv, FaDoorOpen, FaDoorClosed, FaWifi } from 'react-icons/fa';
...
const iconKategori = {
  ...
  parkir: ['Parkir Motor', 'Parkir Mobil', 'Parkir Sepeda', 'Parkir Motor & Sepeda'],
  kasur: ['Kasur'],
  lemari: ['Lemari Baju'],
  kipas: ['Kipas Angin'],
  ac: ['AC'],
          ...
          <div className="p-3">
            ...
            <div className="flex items-center gap-1 mt-1 flex-wrap">
              {Object.entries(iconKategori).map(([kategori, listFasilitas]) => {
                const fasilitasTerpakai = listFasilitas.find((nama) => kost.fasilitas.some((f) => f.nama_fasilitas === nama));

                if (!fasilitasTerpakai) return null;

                const Icon = fasilitasIcons[fasilitasTerpakai];

                return <Icon key={kategori} className="w-4 h-4" title={fasilitasTerpakai} />;
              })}
            </div>
