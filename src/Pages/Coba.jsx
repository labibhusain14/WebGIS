    ...
    <div className="relative h-screen">
      ...
      >
        <div className="flex items-center bg-white ml-2 m-5 rounded-lg p-2 shadow-md w-72">
          <input type="text" placeholder="Search" className="flex-1 bg-transparent border-none outline-none text-sm text-gray-700 placeholder-gray-400" />
              ...
              {activeAssistant === 'assistant' ? (
                <div className="flex flex-col">
                  {messages.map((msg, index) => (
                    <div key={index} className={`mb-3 p-3 rounded-lg max-w-sm ${msg.sender === 'bot' ? 'bg-gray-200 self-start' : 'bg-[#2C3E50] text-white self-end'}`}>
                      <p>{msg.text}</p>
                    </div>
                  ))}
                  <div ref={chatEndRef} />
                  ...
                  <div className="flex overflow-x-auto space-x-4">
                    <div className="flex overflow-x-auto space-x-4">
                      {kosList.map((kos, index) => (
                        <div key={index} className="min-w-[160px] bg-white shadow-md rounded-lg p-2">
                          <img src={kos.image} alt={kos.name} className="h-20 w-full object-cover rounded-md" />
                          <p className="mt-2 text-sm font-semibold">{kos.name}</p>
                          <p className="text-sm text-gray-500">{kos.price}/bulan</p>
                        </div>
                      ))}
                    </div>
                  </div>
